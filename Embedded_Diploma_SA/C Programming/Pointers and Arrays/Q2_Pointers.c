...
int main(){
	...
	Arrange(Numbers,Length);							   //Call Function "Arrange" to sort numbers ascending//
...
void Arrange(uint8 *P_Num, uint8 Length){						//Takes a pointer *p = Numbers//
	...
	for(i = 0; i < Length; i++){
		for(j = i + 1; j < Length; j++){
			if(P_Num[i] < P_Num[j]){							//If the number[1] is bigger than number[0] then swap thw two numbers//
